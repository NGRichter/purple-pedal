...
static char gp_slot_names[GAMEPAD_TOTAL_CURVE_SLOT_NUM][MAX_SLOT_NAME_LEN];
...
int gampepad_setting_handle_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg)
{
    ...
    if (settings_name_steq(name, SETTING_SUB_CALIB_ACTIVE, &next) && !next) {
        if (len != sizeof(uint8_t)) {
            LOG_WRN("NVS Corruption: Invalid length for active calib. Skipping.");
            return 0; 
        }
    ...
    if (settings_name_steq(name, SETTING_SUB_CALIB_SLOT, &next) && next) {
        uint8_t slot = (uint8_t)atoi(next);
        if (slot < 1 || slot > GAMEPAD_TOTAL_CALIB_SLOT_NUM) {
            LOG_WRN("NVS Corruption: Invalid calib slot ID '%s'. Skipping.", next);
            return 0; 
        }
        if (len != sizeof(struct gamepad_calibration)) {
            LOG_WRN("NVS Corruption: Invalid length for calib slot %d. Skipping.", slot);
            return 0; 
        }
    ...
    if (settings_name_steq(name, SETTING_SUB_CURVE"/"SETTING_SUB_CURVE_ACTIVE, &next) && !next) {
        if (len != SETTING_CURVE_ACTIVE_LEN) {
            LOG_WRN("NVS Corruption: Invalid length for active curve. Skipping.");
            return 0;
        }
    ...
    if(settings_name_steq(name, SETTING_SUB_CURVE"/"SETTING_SUB_CURVE_SLOT, &next) && next){
        uint8_t slot = (uint8_t)atoi(next);
        if(slot < 1 || slot > GAMEPAD_TOTAL_CURVE_SLOT_NUM){
            LOG_WRN("NVS Corruption: Invalid curve slot ID '%s'. Skipping.", next);
            return 0; 
        }
        if(len != sizeof(struct gamepad_curve)){
            LOG_WRN("NVS Corruption: Invalid length for curve slot %d. Skipping.", slot);
            return 0;
        }
    ...
    if (settings_name_steq(name, SETTING_SUB_SLOT_NAME, &next) && next) {
        uint8_t slot = (uint8_t)atoi(next);
        if (slot < 1 || slot > GAMEPAD_TOTAL_CURVE_SLOT_NUM) {
            LOG_WRN("NVS Corruption: Invalid name slot ID '%s'. Skipping.", next);
            return 0; 
        }
        ...
        rc = read_cb(cb_arg, gp_slot_names[slot - 1], copy_len);
        return (rc >= 0) ? 0 : rc;
    }
...
int app_setting_init(void)
{
    ...
    for (size_t i = 0; i < GAMEPAD_TOTAL_CURVE_SLOT_NUM; i++) {
        snprintf(gp_slot_names[i], MAX_SLOT_NAME_LEN, "Preset %d", (int)(i + 1));
    }
...
{
    if(slot > GAMEPAD_TOTAL_CURVE_SLOT_NUM){
        return -EINVAL;
    }
...
int set_curve_slot(uint8_t slot_id, const struct gamepad_curve *curve)
{
    ...
    if(slot == 0 || slot > GAMEPAD_TOTAL_CURVE_SLOT_NUM){
        return -EINVAL;
    }
    char setting_name[sizeof(SETTING_CURVE_SLOT) + 4]; 
...
int set_slot_name(uint8_t slot_id, const char *name)
{
    ...
    char setting_name[sizeof(SETTING_SLOT_NAME) + 4]; 
...
int set_calib_slot(uint8_t slot_id, const struct gamepad_calibration *calib)
{
    ...
    char setting_name[sizeof(SETTING_CALIB_SLOT) + 4]; 
